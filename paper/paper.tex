...
\paragraph{Vector Autoregression Series} This dataset consists of autoregressive series of order $p$ which we denote as VAR($d, p$) where $d$ is the feature dimensionality of the series and $p$ is the order of the temporal dependence as before. The series is defined by the recurrence
\begin{align*}
...
Prototype similarities at each layer are shown in Figure \ref{fig:autoregressive_multivariate_dynamics}. Again, the model prefers to diversify later prototype spaces that represent long-term morphologies, and the justification for this is the same as for univariate autoregressive series. However, the learning rate here is noticeably faster than for univariate series -- later prototype diversity increases incredibly rapidly in Stage 1 training. Global training also induces volatility in all prototype layers including the final layer, in contrast to previous experiments.

\begin{figure}
    \centering 
    \includegraphics[width = 0.7\textwidth]{autoregressive_multivariate_dynamics.png}
    \caption{Average pairwise prototype similarity by layer for periodic series representation. Early layer diversification is evident during Stage 3 training (epochs 46-55).}
    \label{fig:autoregressive_multivariate_dynamics}
\end{figure}

PaCMAP embeddings of VAR series representations in two and three dimensions are shown in Figure \ref{fig:autoregressive_multivariate_pacmap}. Unlike for univariate autoregressive series, ProtoTSRL is less able to separate latent temporal dependence structure in multivariate settings. In particular, it distinguishes the simple dependence structure of VAR(1) well, but the complex dependence structures of VAR(5) and VAR(10) are not well-separated. 

\begin{figure}
    \centering 
    \includegraphics[width = 0.95\textwidth]{autoregressive_multivariate_pacmap.png}
    \caption{PaCMAP embeddings of periodic series representations in two and three dimensions.}
    \label{fig:autoregressive_multivariate_pacmap}
\end{figure}

Unlike for univariate periodic data, poor separation in the representation space is more significantly reflected in classification performance (Tables \ref{tab:var_model_accuracies} and \ref{tab:var_confusion_matrix}). As expected ,VAR(1) is easily distinguishable and responsible for much of the observed accuracy of classification models. However, VAR(5) and VAR(10) prove to be difficult for the model to differentiate. Hyperparameter tuning is costly for models of this size, but these results suggest that our chosen method of increasing model complexity and representation capacity is insufficient. Rather than increasing the size and number of prototypes, an alternative approach is to increase the depth of the encoder to allow the model to extract and recognize more complex temporal relationships.
...
\begin{tabular}
    ...
    \hline
    \textbf{Accuracy}
    & \centering 0.776 & \centering 0.750 & \centering 0.793 & \centering\arraybackslash 0.774 \\
    [0.6em]
\end{tabular}
\vspace{0.8em}
\caption{Model accuracies in classification of VAR series representations.}
\label{tab:var_model_accuracies}
\end{table}
\vspace{1em}
...
\setlength{\tabcolsep}{12pt}
\begin{tabular}
    {p{0.20\textwidth} | p{0.16\textwidth} p{0.16\textwidth} p{0.18\textwidth}}
    & \centering\textbf{Predicted VAR(1)} 
    & \centering\textbf{Predicted VAR(5)} 
    & \centering\arraybackslash\textbf{Predicted VAR(10)} \\[0.8em]
    \hline
    \textbf{Actual VAR(1)}  & \centering 340 & \centering 0   & \centering\arraybackslash 0 \\[0.6em]
    \textbf{Actual VAR(5)}  & \centering 0   & \centering 172 & \centering\arraybackslash \textcolor{red}{150} \\[0.6em]
    \textbf{Actual VAR(10)} & \centering 0   & \centering \textcolor{red}{76}  & \centering\arraybackslash 262 \\[0.6em]
\end{tabular}
\vspace{0.8em}
\caption{Confusion matrix for VAR(1), VAR(5), and VAR(10) classification. Incorrect predictions are highlighted in red.}
\label{tab:var_confusion_matrix}
\end{table}
...
\end{tabular}
\vspace{0.8em}
\caption{Model accuracies in classification of PPG data following SFT.}
