...
%% UPDATE TITLE
\title{Interpretable Representation Learning for Time Series}
\author{
    Haiyan Wang \\
    Duke University \\
...
In this paper, we propose a new approach to learning informative, general-purpose representations of time series that is both \textit{interpretable} and \textit{domain-agnostic}. Our primary contributions are: 
\begin{itemize}
    ...
    \item A novel architecture that employs multiscale prototype-based learning to capture both local morphological traits and global temporal dynamics. 
\end{itemize}
...
In this section, we create several synthetic datasets to test the model's ability to represent important characteristics of time series such as periodicity and stationarity. Each dataset consists of time series simulated by  To evaluate the learned representations, we fit a multilayer perceptron 
