from src.models.resblock import ResidualBlock
from src.models.prototypelayer import PrototypeLayer

import torch
import torch.nn as nn
import torch.nn.functional as F



class MultivariateModel(nn.Module):

    def __init__(self, n_channels, representation_dimension, mask_probability = 0.2):

        super().__init__()

        self.n_channels = n_channels
        
        self.representation_dimension = representation_dimension

        self.mask_probability = mask_probability
        
        self.masking_encoder = nn.Sequential(
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : n_channels, 'out_channels' : n_channels*2, 'kernel_size' : 3},
                    {'in_channels' : n_channels*2, 'out_channels' : n_channels*2, 'kernel_size' : 3}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : n_channels*2, 'out_channels' : n_channels*2, 'kernel_size' : 3},
                    {'in_channels' : n_channels*2, 'out_channels' : n_channels*2, 'kernel_size' : 3}
                ]
            )
        )

        self.mask_token = nn.Parameter(torch.randn(1, n_channels*2, 1))

        mask_space_dim = n_channels*2 + 1
        
        self.encoder_layer_1 = nn.Sequential(
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 9, 'out_channels' : 16, 'kernel_size' : 5},
                    {'in_channels' : 16, 'out_channels' : 16, 'kernel_size' : 5}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 16, 'out_channels' : 16, 'kernel_size' : 5},
                    {'in_channels' : 16, 'out_channels' : 16, 'kernel_size' : 5}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 16, 'out_channels' : 32, 'kernel_size' : 5},
                    {'in_channels' : 32, 'out_channels' : 32, 'kernel_size' : 5}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 32, 'out_channels' : 32, 'kernel_size' : 5},
                    {'in_channels' : 32, 'out_channels' : 32, 'kernel_size' : 5}
                ]
            )
        )

        self.prototype_layer_1 = PrototypeLayer(
            n_prototypes = 25,
            prototype_len = 40,
            prototype_channels = 32,
        )

        self.temporal_mixing_layer_1 = nn.Sequential(
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 25, 'out_channels' : 50, 'kernel_size' : 3},
                    {'in_channels' : 50, 'out_channels' : 50, 'kernel_size' : 3}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 50, 'out_channels' : 100, 'kernel_size' : 5},
                    {'in_channels' : 100, 'out_channels' : 100, 'kernel_size' : 5}
                ]
            )
        )

        self.encoder_layer_2 = nn.Sequential(
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 32, 'out_channels' : 64, 'kernel_size' : 5},
                    {'in_channels' : 64, 'out_channels' : 64, 'kernel_size' : 5}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 64, 'out_channels' : 64, 'kernel_size' : 5},
                    {'in_channels' : 64, 'out_channels' : 64, 'kernel_size' : 5}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 64, 'out_channels' : 128, 'kernel_size' : 5},
                    {'in_channels' : 128, 'out_channels' : 128, 'kernel_size' : 5}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 128, 'out_channels' : 128, 'kernel_size' : 5},
                    {'in_channels' : 128, 'out_channels' : 128, 'kernel_size' : 5}
                ]
            )
        )

        self.prototype_layer_2 = PrototypeLayer(
            n_prototypes = 25,
            prototype_len = 40,
            prototype_channels = 128,
        )

        self.temporal_mixing_layer_2 = nn.Sequential(
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 25, 'out_channels' : 50, 'kernel_size' : 3},
                    {'in_channels' : 50, 'out_channels' : 50, 'kernel_size' : 3}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 50, 'out_channels' : 100, 'kernel_size' : 5},
                    {'in_channels' : 100, 'out_channels' : 100, 'kernel_size' : 5}
                ]
            )
        )

        self.encoder_layer_3 = nn.Sequential(
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 128, 'out_channels' : 256, 'kernel_size' : 5, 'dilation' : 2},
                    {'in_channels' : 256, 'out_channels' : 256, 'kernel_size' : 5, 'dilation' : 4}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 256, 'out_channels' : 512, 'kernel_size' : 5, 'dilation' : 8},
                    {'in_channels' : 512, 'out_channels' : 512, 'kernel_size' : 5, 'dilation' : 16}
                ]
            )
        )

        self.prototype_layer_3 = PrototypeLayer(
            n_prototypes = 25,
            prototype_len = 60,
            prototype_channels = 512,
        )

        self.temporal_mixing_layer_3 = nn.Sequential(
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 25, 'out_channels' : 50, 'kernel_size' : 3},
                    {'in_channels' : 50, 'out_channels' : 50, 'kernel_size' : 3}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 50, 'out_channels' : 100, 'kernel_size' : 5},
                    {'in_channels' : 100, 'out_channels' : 100, 'kernel_size' : 5}
                ]
            )
        )

        self.encoder_layer_4 = nn.Sequential(
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 512, 'out_channels' : 512, 'kernel_size' : 5, 'dilation' : 32},
                    {'in_channels' : 512, 'out_channels' : 512, 'kernel_size' : 5, 'dilation' : 64}
                ]
            )
        )

        self.prototype_layer_4 = PrototypeLayer(
            n_prototypes = 20,
            prototype_len = 80,
            prototype_channels = 512,
        )

        self.temporal_mixing_layer_4 = nn.Sequential(
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 20, 'out_channels' : 40, 'kernel_size' : 3},
                    {'in_channels' : 40, 'out_channels' : 40, 'kernel_size' : 3}
                ]
            ),
            ResidualBlock(
                layer_configs = [
                    {'in_channels' : 40, 'out_channels' : 80, 'kernel_size' : 5},
                    {'in_channels' : 80, 'out_channels' : 80, 'kernel_size' : 5}
                ]
            )
        )

        self.last_layer = nn.Linear(
            in_features = 380,
            out_features = representation_dimension
        )

        # model components for stagewise training 
        self.encoder_layers = [self.encoder_layer_1, self.encoder_layer_2, self.encoder_layer_3, self.encoder_layer_4]
        self.prototype_layers = [self.prototype_layer_1, self.prototype_layer_2, self.prototype_layer_3, self.prototype_layer_4]
        self.temporal_mixing_layers = [self.temporal_mixing_layer_1, self.temporal_mixing_layer_2, self.temporal_mixing_layer_3, self.temporal_mixing_layer_4]
        # IMPORTANT: masking_encoder, mask_token, and last_layer are required but accessed directly 

    def forward(self, x):

        z = self.masking_encoder(x)
        if self.training: # only apply masking during training
            mask = self.create_mask(batch_size = z.size(0), seq_len = z.size(-1), device = z.device)
            z = self.apply_latent_mask(z, mask)
        else: # add trivial mask channel during evaluation for consistent input shape
            zero_mask = torch.zeros(z.size(0), 1, z.size(-1), device = z.device, dtype = z.dtype)
            z = torch.cat([z, zero_mask], dim = 1)

        feature_maps = []
        for encoder_layer, prototype_layer, temporal_mixing_layer in zip(self.encoder_layers, self.prototype_layers, self.temporal_mixing_layers):
            z = encoder_layer(z)
            activation_tensor = prototype_layer(z)
            mixed_feature_map = temporal_mixing_layer(activation_tensor)
            pooled_feature_map = F.adaptive_avg_pool1d(mixed_feature_map, 1).squeeze(-1)
            feature_maps.append(pooled_feature_map)

        f = torch.cat(feature_maps, dim = 1)
        out = self.last_layer(f)

        return out
    
    def create_mask(self, batch_size, seq_len, device = None):
        """
        Create a Bernoulli mask over timestamps.

        Returns:
            mask: Float tensor of shape [B, 1, T]
                - 1.0 means 'masked', 0.0 means 'keep'
        """

        if device is None:
            device = self.mask_token.device

        mask = torch.bernoulli(torch.full((batch_size, 1, seq_len), self.mask_probability, device = device))
        
        return mask
    
    def apply_latent_mask(self, z, mask):
        """
        Apply timestamp-level masking in latent space.

        Args:
            z: latent tensor of shape [B, C, T]
            mask: mask tensor of shape [B, 1, T]

        Returns:
            z_out: masked latent tensor, optionally with mask indicator channel
        """
        
        mask = mask.to(dtype = z.dtype, device = z.device)

        z_masked = z * (1.0 - mask) + self.mask_token * mask
        z_masked = torch.cat([z_masked, mask], dim = 1) # add mask indicator channel

        return z_masked